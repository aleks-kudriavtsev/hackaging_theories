...
def test_collect_theories_errors_on_placeholder_pubmed_key(monkeypatch):
    args = argparse.Namespace(
        openalex_api_key=None,
        crossref_api_key=None,
        pubmed_api_key=None,
    ...
    def fake_resolve(config, base_path=None):  # pragma: no cover - patched in test
        assert config == {"pubmed": "cfg"}
        return {"pubmed": "your-pubmed-key"}

    monkeypatch.setattr(collect_theories, "resolve_api_keys", fake_resolve)

    with pytest.raises(collect_theories.MissingSecretError):
        collect_theories._load_api_keys(args, {"pubmed": "cfg"}, base_path=None)
